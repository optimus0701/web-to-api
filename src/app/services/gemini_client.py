...
async def init_gemini_client() -> bool:
    ...
    Priority: Environment variables > config.conf > browser cookies
    ...
    if CONFIG.getboolean("EnabledAI", "gemini", fallback=True):
        try:
            ...
            # Priority 3: Try browser if still not found
            if not gemini_cookie_1PSID or not gemini_cookie_1PSIDTS:
                logger.info("Cookies not found in environment or config. Attempting to retrieve from browser...")
                cookies = get_cookie_from_browser("gemini")
                if cookies:
                    gemini_cookie_1PSID, gemini_cookie_1PSIDTS = cookies
                    logger.info("✅ Retrieved cookies from browser")

            # Get proxy (environment variable takes priority)
            gemini_proxy = os.getenv("HTTP_PROXY") or CONFIG["Proxy"].get("http_proxy") or None
            ...
            if gemini_cookie_1PSID and gemini_cookie_1PSIDTS:
                ...
                error_msg = (
                    "Gemini cookies not found. Please provide them via:\n"
                    "1. Environment variables: GEMINI_COOKIE_1PSID and GEMINI_COOKIE_1PSIDTS\n"
                    "2. config.conf file\n"
                    "3. Login to gemini.google.com in your browser"
                )
